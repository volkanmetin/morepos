...
const handleEdit = async (id) => {
    ...
    try {
        const response = await axios.get(route('category.show', { id: id }));
...
const handleUpdate = async (updatedData) => {
    isLoading.value = true;
    try {
        await axios.put(route('category.update', { id: updatedData.id }), updatedData)
...
const handleCreate = async (createdData) => {
    isLoading.value = true;
    try {
        await axios.post(route('category.store'), createdData)
        dtTable.value.ajax.reload();
...
const handleDelete = async (id) => {
    isLoading.value = true;
    try {
        await axios.delete(route('category.destroy', { id: id }))
        dtTable.value.ajax.reload();
    } catch (error) {
