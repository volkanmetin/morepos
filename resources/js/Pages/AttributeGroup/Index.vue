...
const handleEdit = async (id) => {
    ...
    try {
        const response = await axios.get(route('attribute-group.show', { id: id }));
...
const handleUpdate = async (updatedData) => {
    isLoading.value = true;
    try {
        await axios.put(route('attribute-group.update', { id: updatedData.id }), updatedData)
...
const handleCreate = async (createdData) => {
    isLoading.value = true;
    try {
        await axios.post(route('attribute-group.store'), createdData)
        dtTable.value.ajax.reload();
...
const handleDelete = async (id) => {
    isLoading.value = true;
    try {
        await axios.delete(route('attribute-group.destroy', { id: id }))
        dtTable.value.ajax.reload();
    } catch (error) {
...
const initDataTable = () => {
    const config = createDataTableConfig({
        dataUrl: route('attribute-group.data'),
        columns: columns,
        onEdit: handleEdit,
...
<template>
    <Layout>
        <Head :title="$t('attribute-group.title')"/>
        <PageHeader :title="$t('attribute-group.title')" pageTitle="">
