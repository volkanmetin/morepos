...
const handleEdit = async (id) => {
    ...
    try {
        const response = await axios.get(route('customer.show', { id: id }));
...
const handleUpdate = async (updatedData) => {
    isLoading.value = true;
    try {
        await axios.put(route('customer.update', { id: updatedData.id }), updatedData)
...
const handleCreate = async (createdData) => {
    isLoading.value = true;
    try {
        await axios.post(route('customer.store'), createdData)
        dtTable.value.ajax.reload();
...
const handleDelete = async (id) => {
    isLoading.value = true;
    try {
        await axios.delete(route('customer.destroy', { id: id }))
        dtTable.value.ajax.reload();
    } catch (error) {
...
<template>
    <Layout>
        <Head :title="$t('customer.title')"/>
            <PageHeader :title="$t('customer.title')" pageTitle="">
