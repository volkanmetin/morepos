...
const handleEdit = async (id) => {
    ...
    try {
        const response = await axios.get(route('brand.show', { id: id }));
...
const handleUpdate = async (updatedData) => {
    isLoading.value = true;
    try {
        await axios.put(route('brand.update', { id: updatedData.id }), updatedData)
...
const handleCreate = async (createdData) => {
    isLoading.value = true;
    try {
        await axios.post(route('brand.store'), createdData)
        dtTable.value.ajax.reload();
...
const handleDelete = async (id) => {
    isLoading.value = true;
    try {
        await axios.delete(route('brand.destroy', { id: id }))
        dtTable.value.ajax.reload();
    } catch (error) {
...
<template>
    <Layout>
        ...
        <PageHeader :title="$t('brand.title')" pageTitle="">
